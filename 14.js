...
function minMovesToStables(reindeer, stables) {
  const occupiedStables = []
  return reindeer.reduce((movements, reindeerPos) => {
    const nearestStable = stables
      .toSorted((a, b) => Math.abs(a - reindeerPos) - Math.abs(b - reindeerPos))
      .filter(stable => !occupiedStables.includes(stable))
      .at(0)
    occupiedStables.push(nearestStable)
    movements += Math.abs(reindeerPos - nearestStable)
    return movements
  }, 0)
}
