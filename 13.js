/*
  ...
  *: El movimiento se realiza con el doble de intensidad (ej: *R significa RR)
  ...
  // 'U'  -> se mueve arriba
  // '!U' -> se invierte y se convierte en 'D'
  // '?D' -> no se mueve, ya que ya se hizo el movimiento 
...
function isRobotBack(moves) {
	let mutableMoves = moves
  const position = [0, 0]
  const movementStack = []
  let instructionPointer = 0
  
  const inverseMovements = {
    'L': 'R',
    'R': 'L',
    'U': 'D',
    'D': 'U'
  ...
  while (instructionPointer < mutableMoves.length) {
    const instruction = mutableMoves[instructionPointer]
    ...
    switch (instruction) {
      ...
      case 'D':
        ...
        break
      case '*':
        nextInstruction = mutableMoves[instructionPointer + 1]
        mutableMoves = mutableMoves.substring(0, instructionPointer) + nextInstruction + mutableMoves.substring(instructionPointer + 1)
        instructionPointer--
        break
      case '!':
        nextInstruction = mutableMoves[instructionPointer + 1]
        const inversedInstruction = inverseMovements[nextInstruction]
        mutableMoves = mutableMoves.substring(0, instructionPointer + 1) + inversedInstruction + mutableMoves.substring(instructionPointer + 2)
        break
      case '?':
        nextInstruction = mutableMoves[instructionPointer + 1]
        if (movementStack.includes(nextInstruction)) {
          instructionPointer++
        }
        break
    }
